...
void Instruction(Ins ins) {
    ...
    if (ins.isOpReg(0) && (ins.OpReg(0) == REG_RBP || ins.OpReg(0) == REG_RSP ||
                           ins.OpReg(0) == REG_RIP))
        return;
    if (ins.isOpReg(0) && (ins.OpReg(0) == REG_EBP || ins.OpReg(0) == REG_ESP ||
                           ins.OpReg(0) == REG_EIP))
        return;
    ...
    if (opcount == 2 || (opcount == 3 && opcode >= XED_ICLASS_CMOVB &&
                         opcode <= XED_ICLASS_CMOVZ)) {  // condition mov
        ...
        // cmov in opcode == 2
        if (opcode == XED_ICLASS_XOR && ins.isOpReg(0) && ins.isOpReg(1) &&
            reg_w == reg_r) {        // xor
            InsertCall(ins, reg_w);  // deleteReg
        } else if (ins.isOpReg(0) && ins.isOpReg(1)) {
            ...
            for (Rtn rtn = SEC_RtnHead(sec); rtn.Valid(); rtn = rtn.Next()) {
                ...
                if (isMain || isLib) {
                    RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)function_entry,
                                   IARG_THREAD_ID, IARG_PTR, rtnName, IARG_END);
                    ...
                    if (*rtnName == "read") {
                        ...
                        RTN_InsertCall(
                            ...
                            IARG_END);
                               ...
                               "memcpy") {  // memcpy use xmm registers to copy
                        RTN_InsertCall(
